...
+ V1.2 20230914
  ...
  + 更新程序启动方式，适配到Ubuntu 22.04，抛弃systemd，使用手动执行方式启动；

  + 优化OpenBTS启动脚本，优化启动流程，减小启动错误发生概率；

+ V1.3 20230914

  + 优化数据库重置方式，确保每次启动时数据库都是干净的；
...
```bash
docker pull registry.docker.skygo/addx/gsmsystem-dep:1.3
```

* 容器启动命令

```bash
docker run -dti --privileged --net=bridge  -p 0.0.0.0:8082:8082 -v /dev/bus/usb:/dev/bus/usb --name=gsmsystem gsmsystem-dep:1.3
```
