...
- Base URL / 基地址：`http://HOST:18082/api/v1` (Web default / Web 默认入口);
...
The same `/api/v1` API is available at the Web listener (default `18082`) with
...
同一 API 默认也通过 Web 18082 同源提供，Bearer 规则相同；独立 8082 仅在部署时显式
