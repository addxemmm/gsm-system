...
def config_openbts(id):
    ...
    config = [["1", "540", "1800", "001", "01", "4420", "41240", "test"], \
        ["1", "55", "900", "460", "00", "1", "0", "ChinaMobile"], \
        ["1", "540", "1800", "460", "00", "1", "0", "ChinaMobile"], \
        ["1", "100", "900", "460", "01", "4420", "41240", "ChinaUnicom"], \
        ["1", "722", "1800", "001", "01", "42186", "46980", "ChinaUnicom"]]
...
def set_phone_number(imsi, phone_number):
    status = False
    message_id = 0 
    message = "False"   # 0 -> False, please check /var/log/syslog. 1 -> Success.   2 -> Can not find /var/run/TMSITable.db or /val/lib/asterisk/sqlite3dir/sqlite3.db  3 -> This imsi is not existed.
    ...
    if (os.path.exists(tmsi_table_file) and os.path.exists(asterisk_sqlite3_file)):
        ...
        if (len(tmsi_cursor.fetchall()) != 0 and len(asterisk_cursor.fetchall()) != 0):
            asterisk_cursor.execute("select * from dialdata_table where dial='IMSI" + imsi + "';")
...
def send_smsmessage(imsi, sender, smsmessage):
    ...
    if(len(openbts_pid) == 0 and len(sipauthserve_pid) == 0 and len(smqueue_pid) == 0 and len(asterisk_pid) == 0 and len(transceiver_pid) == 0):
        ...
        os.popen("/OpenBTS/OpenBTSCLI -c sendsms " + str(imsi) + " " + str(sender) + " " + str(smsmessage))
        status = True
        message_id = 1
